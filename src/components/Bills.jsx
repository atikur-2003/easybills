        ...
        {bills.map((bill) => (
          <div key={bill.id} className="flex flex-col md:flex-row gap-7 bg-white shadow-lg p-3 rounded-md">
            <div className="flex justify-center md:justify-start">
                <img className="w-36 h-36 rounded-lg  md:w-[250px] md:h-[250px] " src={bill.icon} alt="" />
            </div>
            <div className="md:mt-8 space-y-2">
              ...
              <p className="text-[#0c2f54]">Due Date: {new Date(bill.due_date).toLocaleDateString()}</p>
              <button
                className="mt-2 md:mt-5 font-semibold bg-[#E0F2FE] text-[#0284C7] px-4 py-2 rounded hover:bg-[#0284C7] hover:text-white"
              > Pay Now
              </button>
            </div>
